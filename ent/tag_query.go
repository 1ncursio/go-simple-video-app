...
func (tq *TagQuery) QueryVideos() *VideoQuery {
	query := &VideoQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, tag.VideosTable, tag.VideosPrimaryKey...),
		)
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withVideos: tq.withVideos.Clone(),
...
func (tq *TagQuery) WithVideos(opts ...func(*VideoQuery)) *TagQuery {
	query := &VideoQuery{config: tq.config}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withVideos; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Tag, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Videos = []*Video{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tag.VideosPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "videos": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "videos" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Videos = append(nodes[i].Edges.Videos, n)
			}
		}
	}
