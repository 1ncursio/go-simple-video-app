...
func (tc *TagCreate) AddVideoIDs(ids ...int) *TagCreate {
	tc.mutation.AddVideoIDs(ids...)
	return tc
}

// AddVideos adds the "videos" edges to the Video entity.
func (tc *TagCreate) AddVideos(v ...*Video) *TagCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return tc.AddVideoIDs(ids...)
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.VideosIDs(); len(nodes) > 0 {
